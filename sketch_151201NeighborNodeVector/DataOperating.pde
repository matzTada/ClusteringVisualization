...
void loadLocationTable() {
  ...
  int i = 0;
  for (TableRow row : locationtable.rows ()) {
    nodes.add(new Node(row.getString("address"), row.getFloat("latitude"), row.getFloat("longitude"), i));
    i++;
  }
  numberOfAvailableNodes = i;
}
...
void saveCurrentNodes() {
  ...
  for (Node tempNode : nodes) { 
    try {
      TableRow newRow = relationtable.addRow();
      newRow.setInt("id", i);
      newRow.setFloat("distanceX", (float)tempNode.distanceX);
      newRow.setFloat("distanceY", (float)tempNode.distanceY);
    }
    catch(NullPointerException e) {
    }
    i++;
