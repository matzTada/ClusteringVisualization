class Path{
  ...
  Node src;
  Node dst;
  ArrayList<Node> memberNodes;
  
  Path(ArrayList<Node> _memberNodes){
    memberNodes = new ArrayList<Node>(_memberNodes);
  }
...
void drawPathDemo() { //demo function to display path. Coordinator and End devices is defined according to the number of neighbor node.
  ...
  int maxNumOfNeighbor = 0;
  int i = 0;
  for (Node tempNode : nodes) {
    println("nodes id: " + i + " numOfNeighbor: " + tempNode.neighborNodes.size());
    if (tempNode.neighborNodes.size() > maxNumOfNeighbor) {//currently in this algorithm, the node which has most neighbornodes and youngest id is chosen
      maxNumOfNeighbor = tempNode.neighborNodes.size();
      co = tempNode;
    }
    if (tempNode.neighborNodes.size() <= 1) {
      tempNode.nodeType = 2;
      ends.add(tempNode);
    }
    i++;
