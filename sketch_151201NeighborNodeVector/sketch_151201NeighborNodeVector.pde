...
void draw() {
  ...
  for (Node tempNode : nodes) {
    tempNode.drawMarker();
    //2015/11/4 added
    tempNode.searchNeighborNodes();
    tempNode.drawNeighborLines();
  }  
  ...
  if (inputmode) {//input mode
    ...
    case '\n': //input_str end
      ...
      if (input_str.length() > 0) {
        ...
        //        }
        for (Node tempNode : nodes) {
          if (input_str.equals(tempNode.address)) foundflag++;
        }
        if (foundflag == 0) {
...
void calcCenterOfGravity() {
  ...
  try {
    ...
    //    }
    for (Node tempNode : nodes) {
      sumbalancelat += tempNode.lat;
      sumbalancelon += tempNode.lon;
    }
  ...
  for (Node tempNode : nodes) {
    //    println(i + " " + nodes[i].distanceX + " " + nodes[i].distanceY);
    tempNode.distanceFromBarycenter = Math.abs(getDistancefromGPSvalue(balancelat, balancelon, tempNode.lat, tempNode.lon));
    tempNode.distanceX = getDistancefromGPSvalue(tempNode.lat, balancelon, tempNode.lat, tempNode.lon);
    tempNode.distanceY = getDistancefromGPSvalue(balancelat, tempNode.lon, tempNode.lat, tempNode.lon);
  }
}
...
void drawDistanceFromCenterOfGravity() {
  ...
  //  }
  for (Node tempNode : nodes) {
    tempNode.drawLine(balancePos.x, balancePos.y);
    fill(0, 0, 100, 200);
    text("" + nfc((float)tempNode.distanceFromBarycenter, 2), (balancePos.x + tempNode.x) / 2, (balancePos.y + tempNode.y) / 2);
    text("(" + nfc((float)tempNode.distanceX, 2) + "," + nfc((float)tempNode.distanceY, 2)+")", tempNode.x, tempNode.y +30);
  }
}
